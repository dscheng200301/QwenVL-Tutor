...
class EduDPODataset(Dataset):
    """
    教育 DPO 偏好数据集
    使用启发式引导 vs 直接给答案 构造偏好对

    偏好设定：
        chosen  = 启发式引导回答（更好）
        rejected = 直接给答案（较差，不利于学习）
    """
    ...
    def __getitem__(self, index: int):
        ...
        # rejected = 简短/不完整的回答（模拟"直接给答案"风格）
        if len(chosen_response) > 60:
            rejected_response = chosen_response[:len(chosen_response) // 3]
            if not rejected_response.strip().endswith((".", "。", "!", "！")):
                rejected_response += "。"
        else:
            rejected_response = chosen_response[:max(10, len(chosen_response) // 4)]
        ...
        chosen_labels = chosen_inputs["input_ids"][0].clone()
        ...
        rejected_labels = rejected_inputs["input_ids"][0].clone()
        ...
        return {
            ...
            "pixel_values": chosen_inputs["pixel_values"][0],
            "image_grid_thw": chosen_inputs["image_grid_thw"][0],
        }
